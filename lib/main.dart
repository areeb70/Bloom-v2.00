...
class GlobalSettings {
  ...
  static Future<void> load() async {
    ...
    // Use SharedPreferences directly (your LocalStorageService doesn't have getThemeColor etc.)
    ...
    // LOAD USER PROFILE FROM LOCAL STORAGE (FIXES BLACK SCREEN)
      ...
      body: ValueListenableBuilder<String>(
        ...
        builder: (context, lang, _) {
          return StreamBuilder<DocumentSnapshot>(
            stream: UserService().getUserStream(widget.user),
            builder: (context, snapshot) {
              // Loading/Error States: Return WIDGETS (Center), NOT Scaffolds
              if (snapshot.connectionState == ConnectionState.waiting)
                return theLoadingScreen(); // Now returns Center()
              ...
              double score = (data['confidenceScore'] ?? 0).toDouble();
...
class _TaskScreenState extends State<TaskScreen> {
  ...
  void _pickRandomTask() async {
    ...
    var userData = await UserService().getUserData(widget.user);
    ...
    return Scaffold(
      appBar: AppBar(
        title: Tr(
          ...
          args: [
            AppTexts.get(
              'level_${widget.levelName.toLowerCase()}',
              GlobalSettings.language.value,
            ),
          ],
      ...
      body: ValueListenableBuilder<String>(
        valueListenable: GlobalSettings.language,
        builder: (context, lang, _) {
          // Get translated task data LIVE using the stored ID
          Map<String, String>? displayTask;
          if (_currentTask != null) {
            displayTask = TaskLibrary.getTaskById(_currentTask!['id']!, lang);
          }
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Tr('all_done', textAlign: TextAlign.center),
                  const SizedBox(height: 30),
                  ElevatedButton(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 20),
                        // LIVE TRANSLATED TASK TITLE
                        Text(
                          displayTask['title'] ?? _currentTask!['title']!,
                          style: const TextStyle(
                            fontSize: 22,
                            fontWeight: FontWeight.bold,
                          ),
                        ...
                        const SizedBox(height: 15),
                        // LIVE TRANSLATED TASK DESC
                        Text(
                          displayTask['desc'] ?? _currentTask!['desc']!,
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            child: Tr(
                              _isCompleted ? 'well_done' : 'i_completed',
  ...
  // MOVE YOUR BODY LOGIC TO A HELPER METHOD ACCEPTING 'lang'
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              "You've completed all challenges in this stage.",
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 30),
            ElevatedButton(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            "Your current challenge for this stage:",
            style: TextStyle(color: Colors.grey[600]),
          ),
          const SizedBox(height: 40),
          Center(
...
class _ReflectionScreenState extends State<ReflectionScreen> {
  ...
  void _finishReflection() async {
    ...
    await UserService().completeTask(widget.user, widget.taskId, points);
    await UserService().saveReflection(widget.user, {
...
class PathLinePainter extends CustomPainter {
  ...
  PathLinePainter({required this.color, required this.progress});

  @override
  void paint(Canvas canvas, Size size) {
    final paintBackground = Paint()
      ..color = color
          .withOpacity(0.1) // Dim background line
      ..strokeWidth = 8
    ...
    // We create the same winding path as the tiles
    // Total distance is roughly 100 * number of milestones
    double stop = 120.0;
    for (int i = 0; i < 6; i++) {
      double x = (i % 2 == 0) ? size.width * 0.2 : size.width * 0.8;
      double y = (i + 1) * stop;
      path.quadraticBezierTo(size.width / 2, y - stop / 2, x, y);
    }

    // 1. Draw the dim background path
    canvas.drawPath(path, paintBackground);

    // 2. Draw the glowing filled path based on progress
    // PathMetric calculates the exact length of the curve
    for (var metric in path.computeMetrics()) {
      final extract = metric.extractPath(0, metric.length * progress);
      canvas.drawPath(extract, paintGlow);
      canvas.drawPath(extract, paintFilled);
    }
  }
...
class MilestoneScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Color primaryColor = GlobalSettings.themeColor.value;
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder<String>(
        ...
        builder: (context, lang, _) {
          return StreamBuilder<DocumentSnapshot>(
            stream: UserService().getUserStream(user),
            builder: (context, snapshot) {
              if (!snapshot.hasData) return theLoadingScreen();

              if (!snapshot.data!.exists) return theLoadingScreen();
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: GlobalSettings.themeColor.value.withOpacity(0.3),
                      blurRadius: 10,
                    ),
                  ],
                ),
